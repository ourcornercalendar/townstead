...
export default function TeamPage() {
  ...
  // Where an invited person actually ends up. It is built from the site
  // address under Site Branding, and without one there is nowhere to send
  // them -- so say that here, before an invite goes out, rather than letting
  // them discover it after accepting.
  const branding = useQuery(
    api.tenantBranding.queries.getByOrgId,
    isReady ? { orgId: orgId! } : "skip"
  );
  const orgSlug = branding?.orgSlug;
  ...
  return (
    <div className="space-y-6">
      ...
      {branding !== undefined && !orgSlug && (
        <div className="rounded-md border border-amber-500/50 bg-amber-500/10 p-4">
          <p className="text-sm font-medium">
            No public site address is set yet.
          </p>
          <p className="mt-1 text-sm text-muted-foreground">
            The event form lives at an address built from it. Until one exists,
            anyone you invite will be able to accept but will have no page to
            add events on. Set it under{" "}
            ...
            <div className="rounded-md border border-green-600/40 bg-green-600/5 p-4 space-y-3">
              ...
              {orgSlug && (
                <p className="text-xs text-muted-foreground">
                  ...
                  <span className="font-mono">/{orgSlug}/events/submit</span> —
                  worth telling them to bookmark it.
                </p>
              )}
            </div>
          )}
